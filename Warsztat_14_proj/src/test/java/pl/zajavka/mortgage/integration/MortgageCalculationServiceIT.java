...
public class MortgageCalculationServiceIT {
    ...
    @Test
    @DisplayName("Whole app calculation works correctly")
    void test() {
        //give, when
        ...
        final var expectedGeneratedContent = readExpectedGeneratedContent();

        for (int i = 0; i < expectedGeneratedContent.size(); i++) {
            Assertions.assertEquals(expectedGeneratedContent.get(i), generatedResultContent.get(i));
        }
    }
    ...
    private List<String> readExpectedGeneratedContent() {
        try {
